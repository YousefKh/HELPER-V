        ...
        if vt_files.get_last_http_error() == vt_files.HTTP_OK:
            ...
            if cat > 0:
               result = 'Hash is Malicious'
                ...
                result = 'Hash is Harmless'
            result = 'Analysis report: ' + result
        else:
    ...
    try:
        ...
        if vt_urls.get_last_http_error() == vt_urls.HTTP_OK:
            ...
            if vt_analyses.get_last_http_error() == vt_analyses.HTTP_OK:
                result = json.loads(result)
            cat = result['data']['attributes']['stats']['malicious']
            if cat > 0:
               result = 'Url is Malicious'
            else:
                result = 'Url is Harmless'
            result = 'Analysis report: ' + result
        else:
...
def get_domain_report(domain, api_key):
    ...
    try:
        ...
        if vt_domain.get_last_http_error() == vt_domain.HTTP_OK:
            ...
            cat = result['data']['attributes']["last_analysis_stats"]['malicious']
            if cat > 0:
               result = 'Domain is Malicious'
            else:
                result = 'Domain is Harmless'
            result = 'Analysis report: ' + result
        else:
...
def main(options):
    ...
    print(" _    _ ______ _      _____  ______ _____     __      __")
    print("| |  | |  ____| |    |  __ \|  ____|  __ \    \ \    / /")
    print("| |__| | |__  | |    | |__) | |__  | |__) |____\ \  / / ")
    print("|  __  |  __| | |    |  ___/|  __| |  _  /______\ \/ /  ")
    print("| |  | | |____| |____| |    | |____| | \ \       \  / ")
    print("|_|  |_|______|______|_|    |______|_|  \_\       \/")
    print('Jou-Kh oct-2020 \n')
    try:
